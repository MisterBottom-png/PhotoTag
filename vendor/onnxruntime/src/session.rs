...
mod dangerous {
    ...
    fn extract_io(
        ...
        let mut type_sys: sys::OnnxEnumInt =
            sys::ONNXTensorElementDataType::ONNX_TENSOR_ELEMENT_DATA_TYPE_UNDEFINED
                as sys::OnnxEnumInt;
        let status = unsafe {
            g_ort().GetTensorElementType.unwrap()(
                tensor_info_ptr,
                &mut type_sys as *mut sys::OnnxEnumInt
                    as *mut sys::ONNXTensorElementDataType,
            )
        };
        ...
        if type_sys
            == sys::ONNXTensorElementDataType::ONNX_TENSOR_ELEMENT_DATA_TYPE_UNDEFINED
                as sys::OnnxEnumInt
        {
            return Err(OrtError::TensorElementType(
                crate::error::OrtApiError::Msg(
                    "ONNX tensor element type is undefined".to_string(),
                ),
            ));
        }
        let io_type = match type_sys {
            x if x
            ...
            _ => {
                return Err(OrtError::UnsupportedTensorElementType {
                    value: type_sys as i32,
                });
            }
        };
